...
console.log(checkPhone("13123456789"))
console.log(checkCode('23456543'))
...
//   if (!checkPhone(phone)) {
//     console.log('Phone has to be at the length of 11')
//     return
//   }

//   if (!checkCode(code)) {
//     console.log("Code has to be at the length of 6")
//     return
//   }

//   console.log('loging in...')
// })
// 因为在 第 10 part重新讲到了这个逻辑，并用了axios，
...
// 第106p 值得一看。教你怎么读文档。有一定难度
// 如何判断 图片被打包的方式：是 1）发送一个单独的文件并导出 URL，还是 2）导出一个资源的 data URI？
//  答： 以 8KB 为临界点
...
document.querySelector('.btn').addEventListener('click', () => {
  ...
  if (!checkPhone(phone)) {
    myAlert(false, 'Phone has to be at the length of 11')
    console.log('Phone has to be at the length of 11')
    return
  }

  if (!checkCode(code)) {
    myAlert(false, 'Code has to be at the length of 6')
    console.log("Code has to be at the length of 6")
    return
  }
...
//       可以通过 在 webpack.config.js 中 设置 devServer: { static: './dist', }, 来将根目录替换成 dist
 ...
 */
import anotherAxios from '@/utils/request.js'
console.log(anotherAxios)
